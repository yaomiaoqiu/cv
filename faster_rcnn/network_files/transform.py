...
class GeneralizedRCNNTransform(nn.Module):
    ...
    def resize(self, image, target):
        ...
        if self.training:
            size = float(self.torch_choice(self.min_size))  # 指定输入图片的最小边长,注意是self.min_size不是min_size，就相当于将我们指定的最小尺寸赋给size
        else:
            # FIXME assume for now that testing uses the largest scale
            size = float(self.min_size[-1])    # 指定输入图片的最小边长,注意是self.min_size不是min_size
        
        # 对图片进行缩放，并处理不合法的缩放因子，保证缩放后的图片的高宽在我们的限制范围内
    ...
    def batch_images(self, images, size_divisible=32):
        # type: (List[Tensor], int) -> Tensor
        """
        将一批图像打包成一个batch返回（注意batch中每个tensor的shape是相同的），主要是为了加快网络的训练速度
        具体操作：获取一批图像中最大的宽和高，然后将所有图片都填充到最大的宽和高
        Args:
            images: 输入的一批图片
    ...
    def forward(self,
        ...
        # 记录resize后的图像尺寸
        # 我们边界框的预测是针对resize之后的，我们要将边界框还原到原图上
        # 又因为我们知道原图的尺寸，记录resize之后的尺寸方便我们将box还原到原图上

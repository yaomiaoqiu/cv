...
class AnchorsGenerator(nn.Module):
    ...
    def generate_anchors(self, scales, aspect_ratios, dtype=torch.float32, device=torch.device("cpu")):
        ...
        # scale 是正方形anchor的基础宽高，aspect_ratios是最终生成anchor的高宽比，需要保证最终anchor和正方形anchor的面积是相同的，
        # 假设正方形anchor的边长为a，最终生成anchor的高宽为h,w
        # a**2=hw，h/w=aspect_ratios
        # w=sqrt(a**2/aspect_ratios), h=w*aspect_ratios
        # w_ratios = w/a = sqrt(1/aspect_ratios) = 1/h_ratios
        # h_ratios = h/a = w*aspect_ratios/a = sqrt(aspect_ratios)
        # h_ratios,w_ratios 是最终生成的anchor的高和宽与基础高和宽的比例，通过比例和基础高和宽，我们就可以得到最终anchor的高宽，并保证最终anchor的面积和基础anchor的面积相同
    ...
    def grid_anchors(self, grid_sizes, strides):
        ...
        for size, stride, base_anchors in zip(grid_sizes, strides, cell_anchors):
            ...
            # 我们将一张图片分成 grid_width*grid_height 份
            shifts_x = torch.arange(0, grid_width, dtype=torch.float32, device=device) * stride_width
            # shape: [grid_height] 对应原图上的y坐标(行)
            shifts_y = torch.arange(0, grid_height, dtype=torch.float32, device=device) * stride_height

            # 计算预测特征矩阵上每个点对应原图上的坐标(anchors模板的坐标偏移量)
            # 图像和特征图都是以矩阵（简化，不考虑通道）的形式表示，此处做法就是将特征图张量的点和图像张量的点对应起来
            # torch.meshgrid函数分别传入行坐标和列坐标，生成网格行坐标矩阵和网格列坐标矩阵
            # shape: [grid_height, grid_width]
            shift_y, shift_x = torch.meshgrid(shifts_y, shifts_x)
            # shift_y 行的值相同，列的值为 shifts_y 的值
            # shift_x 列的值相同，行的值为 shifts_x 的值
            # 总的来说，是一个网格，列是 shifts_y 的值，行是 shifts_x 的值，行代表 shifts_y，列代表 shifts_x
            # 每个网格的小单元格对应特征图的小单元格
            ...
            # 因为我们要将anchors放在原图的每个点上，并且每个点的位置不同，因此我们要记录每个点anchors的偏移量
            shifts = torch.stack([shift_x, shift_y, shift_x, shift_y], dim=1)

            # For every (base anchor, output anchor) pair,
            # offset each zero-centered base anchor by the center of the output anchor.
            # 将anchors模板与原图上的坐标偏移量相加得到原图上所有anchors的坐标信息(shape不同时会使用广播机制)
            # 将 anchors 模板放在每个网格上，中心点和网格左上角重合
            # shifts_anchor [ceil_num, anchor_num, 4]  4 代表anchor坐标
            # 看一下anchor的形状，结合anchors.png图片理解
            # 坐标原点在图片的左上角，坐标原点是不变的，anchor的四个参数根据anchor的位置会不断变换，anchor的中心是在网格的每个顶点
    ...
    def forward(self, image_list, feature_maps):
        ...
        # 第一个 list 每个元素为每张图片，第二个list每个元素为每个预测特征层的anchors
...
def concat_box_prediction_layers(box_cls, box_regression):
    ...
    for box_cls_per_level, box_regression_per_level in zip(box_cls, box_regression):
        ...
        # 注意，当计算RPN中的proposal时，classes_num=1, 即C=1，只区分目标和背景
        N, AxC, H, W = box_cls_per_level.shape
        # # [batch_size, anchors_num_per_position * 4, height, width]
        Ax4 = box_regression_per_level.shape[1]
        # anchors_num_per_position，4是anchor的4个回归参数
        A = Ax4 // 4
        # classes_num
        C = AxC // A

        # 为什么要进行维度的调整？ 方便将预测值和训练的anchor进行结合，后面进行proposal过滤时也会更方便

        # [N, -1, C] 此处 C 为分类数
        box_cls_per_level = permute_and_flatten(box_cls_per_level, N, A, C, H, W)
        box_cls_flattened.append(box_cls_per_level)

        # [N, -1, C] 此处 C 为anchor的4个参数， -1 代表anchor的数目
        box_regression_per_level = permute_and_flatten(box_regression_per_level, N, A, 4, H, W)
        box_regression_flattened.append(box_regression_per_level)

    # 将多个预测特征层上的anchor进行拼接，然后展平，展平后 batch 和 anchor结合在一起形成 [batch * anchor, C]
...
class RegionProposalNetwork(torch.nn.Module):
    ...
    def _get_top_n_idx(self, objectness, num_anchors_per_level):
        ...
        offset = 0
        # 遍历每个预测特征层上的预测目标概率信息
        # objectness [num_image, batch * all_anchor * C / num_image]  ?
        # objectness.split: 将objectness在 1 维分割，分割长度为num_anchors_per_level
        for ob in objectness.split(num_anchors_per_level, 1):
            ...
            # 返回的是每一层的topk的索引，索引都是从 0 开始的
            _, top_n_idx = ob.topk(pre_nms_top_n, dim=1)
            r.append(top_n_idx + offset)
            # 每一层anchor的数目不一样，我们要记录偏移量来偏移获取的排序好的索引，确保其对应的是每一层的索引
            offset += num_anchors
        # 对 list 中的 tensor 进行拼接，是在pre_nms_top_n 维度拼接
    ...
    def filter_proposals(self, proposals, objectness, image_shapes, num_anchors_per_level):
        ...
        Args:
            proposals: 预测的bbox坐标  [num_image, anchors/num_image, 4]
            objectness: 预测的目标概率  [batch * all_anchor, C]
        ...
        # 为什么不进行反向传播，因为后面得到的 proposals 是fast rcnn的输入参数，不是模型计算得到的中间变量，可以理解为一个叶子节点，并且require_grad=False
        objectness = objectness.detach()
        # [num_image, batch * all_anchor * C / num_image]
        objectness = objectness.reshape(num_images, -1)

        # Returns a tensor of size size filled with fill_value
        # levels负责记录分隔不同预测特征层上的anchors索引信息
        # levels 是个列表，每个元素都是一个tensor，tensor 值为用 idx 填充的 n 个元素
        # 假设 num_anchors_per_level = [19380, 2000]，
        # 那么 levels = [tensor([0,0,0,....]), tensor([1,1,1...])] ,一共有 19380 个 0，2000个 1
        levels = [torch.full((n, ), idx, dtype=torch.int64, device=device)
                  for idx, n in enumerate(num_anchors_per_level)]
        # [sum(num_anchors_per_level)]  [0,0,...,0,1,1,...,1]
        # levels 相当于一个 mask，通过levels我们可以知道哪些anchors是属于哪些特征层的
        levels = torch.cat(levels, 0)

        # Expand this tensor to the same size as objectness
        # reshape: [1, sum(num_anchors_per_level)]
        levels = levels.reshape(1, -1).expand_as(objectness)

        # select top_n boxes independently per level before applying nms
        # 获取每张预测特征图上预测概率排前pre_nms_top_n的anchors索引值
        # [num_image, top_n_anchors_idx]
        ...
        for boxes, scores, lvl, img_shape in zip(proposals, objectness_prob, levels, image_shapes):
            ...
            # 此操作返回的是进行nms处理后，按照scores排序好的索引
    ...
    def compute_loss(self, objectness, pred_bbox_deltas, labels, regression_targets):
        ...
        """
        # 按照给定的batch_size_per_image, positive_fraction选择正负样本，我们并不使用所有的正负样本
    ...
    def forward(self,
        ...
        # 计算每个预测特征层上的对应的anchors数量，len(objectness) 对应预测特征层的个数，mobilenetv2 为 1个预测特征层
        # objectness 每一个元素都是一个预测特征层矩阵[batch, anchor_num_per_ceil, height, width]，
        # o[0].shape 对应[anchor_num_per_ceil, height, width]
        num_anchors_per_level_shape_tensors = [o[0].shape for o in objectness]
        # 特征矩阵的每个ceil都会生成15个anchors，一个有 height * width 个ceil
        num_anchors_per_level = [s[0] * s[1] * s[2] for s in num_anchors_per_level_shape_tensors]

        # 调整内部tensor格式以及shape [batch * all_anchor, C]
        ...
        # proposals [anchors, 1, 4] --> [num_images, anchors/num_images, 4]

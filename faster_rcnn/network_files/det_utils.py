...
def encode_boxes(reference_boxes, proposals, weights):
    ...
    # 通过公式计算的 gt 相对于 proposal 的回归参数（回归偏移量）
...
class BoxCoder(object):
    ...
    def decode(self, rel_codes, boxes):
        ...
        # boxes [batch, anchors, anchors_param]，记录每张图片的anchors数目
        # 不同批次图片的 anchors 数目可能不同，因为我们之前预处理时，是将图片填充为这一批图片中最大的高和宽，因为没批图片的高宽不同，这会导致anchor的不同
        boxes_per_image = [b.size(0) for b in boxes]
        # 将所有 batch 的anchor拼接在一起 [batch * anchors, 4]
        concat_boxes = torch.cat(boxes, dim=0)

        # 此处是为了获取这一批图片的anchor总数
        box_sum = 0
        for val in boxes_per_image:
            box_sum += val

        # 将预测的bbox回归参数应用到对应anchors上得到预测bbox的坐标 [anchors, 4]
    ...
    def decode_single(self, rel_codes, boxes):
        ...
        # 这部分根据anchor的信息和预测的边界框回归参数来获取最终的proposal参数（PPT公式），因为 dx是二维的，因此我们要给其它参数添加一个维度
        pred_ctr_x = dx * widths[:, None] + ctr_x[:, None]
        pred_ctr_y = dy * heights[:, None] + ctr_y[:, None]
        pred_w = torch.exp(dw) * widths[:, None]
        pred_h = torch.exp(dh) * heights[:, None]

        # 将获取到的proposal中心点坐标，宽和高转换为左上角坐标和右下角坐标的形式
        # xmin [anchors, 1]
        ...
        # stack拼接会新增一个维度，cat不会新增维度，pred_boxes [anchors, 1, 4] --> [anchors, 4]
...
class Matcher(object):
    ...
    def set_low_quality_matches_(self, matches, all_matches, match_quality_matrix):
        ...
        quality value.
        此函数主要是为了防止有的 gt 没有被 anchor 所匹配到，我们将每个 gt-anchor 的最大IOU对应的 anchor 也记为正样本
        """
        ...
        # 记录了每个 gt 对应的最大IOU的 anchor 的索引
        pre_inds_to_update = gt_pred_pairs_of_highest_quality[1]
        # 保留该anchor匹配gt最大iou的索引，即使iou低于设定的阈值
        # matches 中记录了每个anchor是正样本还是负样本，如果是正样本则为对应的gt索引
        # all_matches 中记录了 gt-anchor 矩阵中每列的最大值的gt索引
